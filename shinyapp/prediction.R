...
PredGram <- function(input, nOutput = 15) {
    ...
    if (nrow(tbResult3) == 1 & is.na(as.vector(tbResult3[, "y", with = F][1])) == T) {
        ...
        if (nrow(tbResult2) == 1 & is.na(as.vector(tbResult2[, "y", with = F][1])) == T) {
            ...
            if (nrow(tbResult1) == 1 & is.na(as.vector(tbResult1[, "y", with = F][1])) == T) {
                ## sort the result
                tbSortedResult0 <- head(tbUnigram[, c("y", "prob"), with = F][order(-rank(prob), y)], nOutput)
                
                return(data.frame(tbSortedResult0)) ## return Unigram result ##
            } else {
                ## sort the result
                tbSortedResult1 <- head(tbResult1[order(-rank(prob), y)], nOutput)
                
                return(data.frame(tbSortedResult1)) ## return Bigram result ##
            }
        } else {
            ## sort the result
            tbSortedResult2 <- head(tbResult2[order(-rank(prob), y)], nOutput)
            
            return(data.frame(tbSortedResult2)) ## return Trigram result ##
        }
    } else {
        ## sort the result
        tbSortedResult3 <- head(tbResult3[order(-rank(prob), y)], nOutput)
        
        return(data.frame(tbSortedResult3)) ## return Quatrgram result ##
    }
